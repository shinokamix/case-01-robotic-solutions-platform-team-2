...
Платформа использует модульный монолит, один backend и одну транзакционную базу. Экономика, рекомендации и имитация остаются отдельными модулями Go-приложения. Для MVP микросервисы не нужны.

Backend пишется на Go. Код для подготовки данных и экспериментов ML находится в отдельной Python-зоне монорепозитория. Основной пользовательский путь не зависит от ML.
...
Frontend работает в браузере как одностраничное приложение на React и TypeScript. Проект использует Vite+ как единый набор инструментов для разработки, форматирования, линтинга, проверки типов, тестов и сборки. Команды выполняются через CLI `vp`, а локальная зависимость называется `vite-plus`. Production-сборка содержит статические файлы, а браузер обращается к Go API. SSR не входит в архитектуру, поэтому отдельный frontend-сервер не нужен.

| Область | Решение | Причина |
| --- | --- | --- |
| Frontend | React, TypeScript, Vite+ | Клиентское приложение без SSR и с типизированным API-клиентом |
...
Конкретные библиотеки для маршрутизации, доступа к данным и миграций нужно закрепить отдельным техническим решением.
...
packages/
  ...
  config/                # общая конфигурация frontend и Vite+
data/
...
- frontend `vp check`, `vp test` и `vp build`;
